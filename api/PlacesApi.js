...
export async function getVerifiedPlaces(placesRetrieved, location, distance) {
  ...
  for (const b of bounds) {
    await firestore.collection("Places").where("isValidated","==",true).orderBy('geohash').startAt(b[0]).endAt(b[1]).get().then((snapshot) => {
...
export async function updatePlace() {
  ...
  for(var i = 0; i < placeList.length; i++) {
    ...
    .set({
      ...
      geohash: geofire.geohashForLocation([placeList[i].data().location.latitude, placeList[i].data().location.longitude]),
...
export async function getLikedPlacesNoUser(placesRetrieved) {
  ...
  if(data.data().likedPlaces.length > 0) {
      for(var i = 0; i < data.data().likedPlaces.length; i++) {
        await firestore.collection("Places").where("name","==", data.data().likedPlaces[i]).get().then(snapshot => {
          snapshot.forEach(doc => {
            likedPlaces.push(doc);
          })
...
export async function addLiked(placeName) {
  ...
  firestore.collection("Users").doc(uid)
    ...
    .catch((error) => console.log(error));
  firestore.collection("Places").where("name", "==", placeName).get().then(snapshot => {
    snapshot.forEach(doc => {
      firestore.collection("Places").doc(doc.id).update({
...
export async function getMostPopularPlaces(onPopularPlacesReceived) {
  var popularPlaces = []
  await firestore.collection("Places").orderBy("numberOfLikes","desc").limit(10).get().then(snapshot => {
    snapshot.forEach(doc => {
